...
- **Messages is now a full desktop app (icon), with two conversations.** A **VESPER** thread and the **house channel (████ / Helena)** live in a chat-client two-pane layout.
  ...
  - **House channel (Helena):** the live-intrusion branching tree (below) now lives as the second conversation; it is dormant ("offline / no recent messages") until the co-user event brings it live, then the player must answer as Amelia.
